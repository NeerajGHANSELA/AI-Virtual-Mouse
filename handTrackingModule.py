...
class handDetector():
    ...
    def findPosition(self, image, handNo=0, draw=True):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, lm in enumerate(hand.landmark):
                ...
                pixel_x, pixel_y, pixel_z = int(lm.x * width), int(lm.y * height), int(lm.z * width)
    ...
    def trackingFingersUpOrClosed(self, image, lmList):
        """
            This function returns a list of which fingers are open and which are closed.
            1: open
            0: closed
        """
    ...
    def findDistance(self, p1, p2, image, lmList, draw=True, radius=15, thickness=3):
        p1_x, p1_y = lmList[p1][1:-1]
        p2_x, p2_y = lmList[p2][1:-1]

        # centre point on the line between the two points
        cx, cy = (p1_x + p2_x) // 2, (p1_y + p2_y) // 2

        if draw:
            # draw the line between the two points
            cv2.line(image, (p1_x, p1_y), (p2_x, p2_y), (255, 0, 255), thickness)

            # draw a circle around the p1
            cv2.circle(image, (p1_x, p1_y), radius, (255, 0, 255), cv2.FILLED)
            # draw the circle  aorund the p2
            cv2.circle(image, (p2_x, p2_y), radius, (255, 0, 255), cv2.FILLED)

            # draw the circle at the centre point
            cv2.circle(image, (cx, cy), radius, (0, 0, 255), cv2.FILLED)

        length = math.hypot(p2_x - p1_x, p2_y - p1_y)

        return length, image, [p1_x, p1_y, p2_x, p2_y, cx, cy]
